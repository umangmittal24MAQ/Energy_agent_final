...
# ──────────────────────────────────────────────────────────────────────────────
# Meter OCR Engine Endpoints
# ──────────────────────────────────────────────────────────────────────────────

# 🟢 PUBLIC (Read-Only)
@router.get("/scheduler/meter-ocr/status")
async def meter_ocr_status(current_user: dict = Depends(get_current_user)) -> Dict[str, Any]:
    """Returns the current status of the Meter OCR engine subprocess."""
    return get_meter_ocr_status()


# 🔴 RESTRICTED (Admins Only)
@router.post("/scheduler/meter-ocr/start")
async def meter_ocr_start(admin_user: dict = Depends(verify_admin)) -> Dict[str, Any]:
    """Manually starts the Meter OCR engine outside of its scheduled window."""
    from app.services.scheduler_service import _start_meter_ocr_engine
    _start_meter_ocr_engine()
    return {"status": "start requested", **get_meter_ocr_status()}


# 🔴 RESTRICTED (Admins Only)
@router.post("/scheduler/meter-ocr/stop")
async def meter_ocr_stop(admin_user: dict = Depends(verify_admin)) -> Dict[str, Any]:
    """Manually stops the Meter OCR engine."""
    from app.services.scheduler_service import _stop_meter_ocr_engine
    _stop_meter_ocr_engine()
    return {"status": "stop requested", **get_meter_ocr_status()}
