...
def send_daily_report(
    ...
    try:
        ...
        master_df = sp_service.fetch_sheet_data("grid_and_diesel")
        ...
        if manual_date:
            target_dt = pd.to_datetime(manual_date, errors="coerce")
            if pd.isna(target_dt):
                row_df = master_df[date_series.dt.strftime("%Y-%m-%d") == manual_date]
                report_date = manual_date
            else:
                row_df = master_df[date_series.dt.date == target_dt.date()]
                report_date = target_dt.strftime("%Y-%m-%d")
            ...
            if valid_dates.empty:
                ...
                row_df = master_df[date_series.dt.date == latest_dt.date()]
                day_data = row_df.iloc[-1].to_dict() if not row_df.empty else master_df.iloc[-1].to_dict()
                report_date = latest_dt.strftime("%Y-%m-%d")

        # --- Inject HTML EXACTLY as specified ---
        try:
            body_html = _build_strict_email_html(master_df, report_date, custom_message=status_note or "")
...
def send_operator_reminder() -> Dict[str, Any]:
    try:
        ...
        reminder_to = os.getenv("OPERATOR_EMAIL", "ojas.upadhyay@maqsoftware.com")
        to_list = [e.strip() for e in str(reminder_to).split(",") if e.strip()]
        if not to_list:
            to_list = ["ojas.upadhyay@maqsoftware.com"]

        now_ist = datetime.now(ZoneInfo("Asia/Kolkata"))
        today_disp = now_ist.strftime("%d-%b-%Y")
        prev_disp = (now_ist - timedelta(days=1)).strftime("%d-%b-%Y")

        deadline_str = "10:30"
        try:
            deadline_disp = datetime.strptime(deadline_str, "%H:%M").strftime("%I:%M %p")
        except ValueError:
            deadline_disp = "10:30 AM"

        subject = "Action Required: Grid and Diesel Data Missing"
        body = (
            f"Hi Ojas,\n\n"
            f"The system noticed that the Grid and Diesel data has not been logged in today's row ({today_disp}).\n"
            f"Please log the consumption data for {prev_disp} into the '{today_disp}' row in the 'Electrical_Optimization.xlsx' file on SharePoint.\n\n"
            f"This must be completed before {deadline_disp} so the Daily Energy Report can be generated.\n\n"
        ...
        return {
            "status": "Success",
            "notes": "Operator reminder sent",
            "recipients": ", ".join(to_list),
        }
    except Exception as e:
        return {"status": "Failed", "error": str(e)}

def send_data_fetch_failure_alert() -> Dict[str, Any]:
    try:
        from app.services.scheduler_service import load_scheduler_config
        sched_config = load_scheduler_config()
        ...
        to_emails_str = sched_config.get("to", os.getenv("OPERATOR_EMAIL", "umang.mittal@maqsoftware.com"))
        cc_emails_str = sched_config.get("cc", "")
        ...
        subject = "Action Required: Data Source Could Not Be Fetched"
        body = (
            "Hello,\n\n"
            "The automated energy scheduler could not fetch source data from SharePoint.\n"
            "Please check SharePoint availability, authentication, and source file access.\n"
            "After resolution, the next scheduler cycle will generate the final report mail automatically.\n\n"
            "Thank you,\n"
            "Energy Automation Agent"
        )
        ...
        return {
            "status": "Success",
            "notes": "Data fetch failure alert sent",
