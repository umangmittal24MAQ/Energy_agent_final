...
def _is_fault(status: Any) -> bool:
    return str(status).strip().upper() == "FAULT"
...
def get_today_uptime_from_sheet() -> Dict[str, Any]:
    ...
    Each 30-min row where status == ACTIVE counts as +30 mins uptime,
    FAULT counts as +30 mins downtime, NaN is skipped.
